  ...
  return (
    <section className="bg-green-pale border-t border-b border-[#D1FAE5] py-24">
      <div className="max-w-6xl mx-auto px-6 md:px-10">
        <p className="text-[11px] font-bold text-ink-4 uppercase tracking-[.12em] mb-3.5">How it works</p>
        ...
        <div className="relative grid grid-cols-1 md:grid-cols-3 gap-12 md:gap-0">
          ...
          <div
            className="hidden md:block absolute top-[26px] pointer-events-none"
            style={{
              left: "calc(16.67% + 26px)",
              right: "calc(16.67% + 26px)",
              height: "2px",
              background: "repeating-linear-gradient(90deg, #A7F3D0 0, #A7F3D0 8px, transparent 8px, transparent 16px)",
              zIndex: 0,
            }}
          />
          ...
          {steps.map((s) => (
            <div key={s.title} className="text-center relative z-10">
              <div className={`w-[52px] h-[52px] rounded-full ${s.bg} grid place-items-center mx-auto mb-5 transition-transform hover:scale-110`}>
                {s.icon}
              </div>
              <h3 className="text-[17px] font-extrabold text-ink mb-2 tracking-[-0.1px]">{s.title}</h3>
              <p className="text-[14px] text-ink-3 leading-[1.65] max-w-[26ch] mx-auto">{s.desc}</p>
            </div>
          ))}
        </div>
      </div>
